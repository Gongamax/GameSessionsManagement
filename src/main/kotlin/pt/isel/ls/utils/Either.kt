...
fun <L, R, T> handleEither(
    either: Either<L, R>,
    onSuccess: (R) -> Either<L, T>,
): Either<L, T> =
    when (either) {
...
fun <F> handleConditions(
    conditions: List<Condition<F>>,
    onSuccess: () -> Unit,
): Either<F, Unit> = conditions.find { it.predicate }?.let { failure(it.failure) } ?: success(onSuccess())
