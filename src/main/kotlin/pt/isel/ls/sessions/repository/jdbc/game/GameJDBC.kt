...
class GameJDBC(
    ...
    override fun createGame(name: String, developer: String, genres: List<Genres>): UInt? =
        dataSource.connection.execute("Failed to create game") { con ->
            ...
            val stm = con.prepareStatement(query, Statement.RETURN_GENERATED_KEYS).apply {
                setString(1, name)
    ...
    override fun getGames(genres: List<Genres>, developer: String, skip: Int, limit: Int): List<Game> =
        dataSource.connection.execute("Failed to get games") { con ->
            ...
            val genreOrdinals = getGenreOrdinals(genres)
            val stm = con.prepareStatement(query).apply {
    ...
    override fun getGameById(gid: UInt): Game? = dataSource.connection.execute("Failed to get game") { con ->
    ...
    override fun reset() = dataSource.connection.execute("Failed to reset") { con ->
    ...
    private fun getGenreOrdinals(genres: List<Genres>) =
        genres.map { it.ordinal }.toTypedArray()
}